...
Where CATEGORY and IDENTIFIER are placeholders.
...
| `.signature` | Signature only | `$SYM_VALIDATE.signature` → `"(token: string) => Promise"` | Type checking |
